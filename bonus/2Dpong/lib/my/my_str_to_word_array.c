...
** File description:
** Splits a string into words. Separators will all be non-alphanumeric characters. The function returns an array in which each cell contains the address of a string (representing a word). The last cell must be null to terminate the array.
*/
...
{
	if (isalph(str[n]) == 1 && isalph(str[n + 1]) == 0)
		return (1);
	else
		return (0);
}
...
char **my_str_to_word_array(char *str)
{
	...
	for (i = 0 ; str[i] != '\0' ; i++) {
		if (isalph(str[i]) == 1 && (i == 0 || isalph(str[i - 1]) == 0))
		{
